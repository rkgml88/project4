            ...
            <div class="main">
                ...
                <div class="postWrap">
				    <c:choose>
				        ...
				        <c:otherwise>
				            <div id="postExist">
				                <c:forEach var="post" items="${coffeeList}">
				                    <div class="post">
			                        	<div class="imgWrap">
                ...
                <div id="bottomRow">
					<div id="pagination">
					    <!-- 첫 페이지 이동 버튼 -->
					    <c:if test="${pageCoffee > 1}">
					      <a class="jump" href="?pageCoffee=1">
					    ...
					    <c:forEach begin="${startPage}" end="${endPage}" var="p">
					      <c:choose>
					        <c:when test="${p == pageCoffee}">
					          <span id="nowp">${p}</span> <!-- 현재 페이지는 굵게 표시 -->
					        </c:when>
					        <c:otherwise>
					          <a class="nonp" href="?pageCoffee=${p}">${p}</a>
					        </c:otherwise>
					      </c:choose>
					    </c:forEach>
					
					    <!-- 마지막 페이지 이동 버튼 -->
					    <c:if test="${pageCoffee < coffeeTotalPages}">
					      <a class="jump" href="?pageCoffee=${coffeeTotalPages}">
                    ...
                    <!-- 글쓰기 버튼 -->
                    <sec:authorize access="hasRole('ROLE_ADMIN')">
                    	<button id=writeBtn><a href="write?category=Menu&subCategory=커피">글쓰기</a></button>
                    </sec:authorize>
                </div>
            </div>
        </section>
