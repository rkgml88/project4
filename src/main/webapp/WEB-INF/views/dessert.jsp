    ...
    <div class="wrap">
        ...
        <section>
            ...
            <div class="main">
                ...
                <div class="postWrap">
				    <c:choose>
				        <c:when test="${empty dessertList}">
				            <div align="center">등록된 게시물이 없습니다.</div>
				        </c:when>
				        <c:otherwise>
				            <div id="postExist">
				                <c:forEach var="post" items="${dessertList}">
				                    <div class="post">
			                        	<div class="imgWrap">
                ...
                <div id="bottomRow">
					<div id="pagination">
					    <!-- 첫 페이지 이동 버튼 -->
					    <c:if test="${pageDessert > 1}">
					      <a class="jump" href="?pageDessert=1">
					    ...
					    <c:forEach begin="${startPage}" end="${endPage}" var="p">
					      <c:choose>
					        <c:when test="${p == pageDessert}">
					          <span id="nowp">${p}</span> <!-- 현재 페이지는 굵게 표시 -->
					        </c:when>
					        <c:otherwise>
					          <a class="nonp" href="?pageDessert=${p}">${p}</a>
					        </c:otherwise>
					      </c:choose>
					    </c:forEach>
					
					    <!-- 마지막 페이지 이동 버튼 -->
					    <c:if test="${pageDessert < dessertTotalPages}">
					      <a class="jump" href="?pageDessert=${dessertTotalPages}">
